//! Image editor CLI with parallel CPU-bound processing.
...
fn build_uploader() -> Arc<dyn FileUploader> {
    match std::env::var("MYME_UPLOADER").as_deref() {
        Ok("s3") => {
            ...
            Arc::new(S3Uploader { client, bucket, runtime })
        }
        _ => {
...
fn fetch_bytes(line: &str) -> Result<Vec<u8>, AppError> {
    if line.starts_with("http://") || line.starts_with("https://") {
        Ok(http_client()?.get(line).send()?.bytes()?.to_vec())
    } else {
        Ok(fs::read(line)?)
    }
}
...
fn process_image(
    ...
    let bytes = match fetch_bytes(line) {
        Ok(b) => b,
        Err(e) => {
        ...
        .and_then(|r| Ok(r.decode()))
    {
        Ok(Ok(img)) => img,
        Ok(Err(e)) => {
            eprintln!("[{line}] decode error: {e}");
            return;
        }
        Err(e) => {
            eprintln!("[{line}] format error: {e}");
            return;
        }
    };
...
fn main() {
    ...
    if args.sequential {
        println!("[mode] sequential, {} files", lines.len());
        for (i, line) in &lines {
            process_image(
                line,
                width,
        ...
        println!(
            "[mode] parallel ({} threads), {} files",
            rayon::current_num_threads(),
            lines.len()
        );
        lines.par_iter().for_each(|(i, line)| {
            process_image(
                line,
                width,
                height,
